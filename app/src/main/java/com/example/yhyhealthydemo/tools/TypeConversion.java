...
/**
 * 類型轉換
 * 1、bytes2HexString(byte[] b, int length) 將字節數組-->16進製字符串 有空格
 * 2、hexString2Bytes(String src) 16進製字符串-->字節數組
 * 3、string2HexString(String strPart) 普通字符串-->16進製字符串
 * 4、hexString2String(String src) 16進製字符串-->普通字符串
 * 5、char2Byte 字符-->字節數據
 * 6、intToHexString(int a,int len) 10進制數字-->轉成16進製字符串
 * 7、stringToByteArray(String s) 普通字符串-->字節數組
 * 8、bytes2HexString(byte[] b） 將字節數組-->16進製字符串 沒有空格
 * 9、bytes20xHexString(byte[] b） 將字節數組-->16進製字符串 有0x標記
 *
 * asciiStr2HexStr(String ascStr) 將ASCII編碼的字符串轉換成對應ASCII碼值16進制數組成的字符串
 */
